...
// A dedicated, lightweight logos index (channel id -> logo URL) - fetched
// the same way as channels/streams/feeds, not a full repo/database ZIP
// download (that's not something a serverless function can do, and isn't
// needed: this single JSON file already covers ~35k channels).
const IPTV_ORG_LOGOS_URL = process.env.IPTV_ORG_LOGOS_URL || "https://iptv-org.github.io/api/logos.json";
...
// Most rows in channels.json have no populated `logo` field at all - this
// separate index is where iptv-org actually keeps them (sourced in part
// from tv-logo/tv-logos and others), one channel id mapping to several
// format/feed variants. Picks the best one per channel: the main feed
// (not a regional/HD variant), marked still in_use, PNG preferred over
// SVG for the widest compatibility.
async function fetchChannelLogos() {
  try {
    const rows = await resilientFetch(
      "iptv-org:logos",
      IPTV_ORG_LOGOS_URL,
    ...
    const scoreOf = (row) => (row.feed ? 0 : 1) + (row.in_use ? 1 : 0) + (String(row.format).toUpperCase() === "PNG" ? 1 : 0);
    const byChannel = new Map();
    ...
    return new Map(Array.from(byChannel, ([channel, row]) => [channel, row.url]));
  } catch {
    return new Map();
  }
}
...
export async function fetchUkChannelsFromIptvOrg() {
  ...
  const [streamData, languageLookup, logoLookup] = await Promise.all([
    fetchUkStreamData(),
  ...
  for (const row of rows) {
    ...
    channels.push({
      ...
      watchVia: [],
      logo: row.logo || logoLookup.get(row.id) || null,
      ...
      // Our own server-side proxy/retry can only make a geo-blocked stream
      // WORSE (it runs from a fixed, non-UK Vercel region) - the browser's
      // own request, from the visitor's real location, is the only attempt
      // that has any chance of working, so this needs to be known upfront.
      streamGeoBlocked:
        streamUrl === matchedStream?.url &&
        (matchedStream?.labels || []).some((label) => String(label).toLowerCase() === "geo-blocked"),
        ...
        ? [
            {
              ...
              geoBlocked: (matchedStream.labels || []).some((label) => String(label).toLowerCase() === "geo-blocked"),
              status: "unknown",
              quality: "unknown",
              source: "iptv-org"
            }
...
export async function fetchScheduleFromXmlTv(xmlUrl, countryCode = "GB") {
  ...
  while ((match = programmeRegex.exec(xml)) !== null && items.length < XMLTV_ITEM_LIMIT) {
    ...
    items.push({
      ...
      show: title,
      channel: channelName,
      // Freeview-EPG's per-programme <icon> is frequently a shared,
      // low-effort placeholder graphic reused across many unrelated
      // programmes (confirmed: several distinct icon URLs resolve to
      // byte-identical images), including ones with text like "Content
      // not viewable in your region" baked directly into the pixels -
      // rendering that as a show's artwork looked broken, not helpful.
      // Leave it out entirely; the channel logo is the reliable fallback.
      image: null,
      summary,
      startAt,

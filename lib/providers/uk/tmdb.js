 ...
 *
 * TMDB actually returns every country's availability in one response - this
 * used to discard everything except GB, which meant no amount of deep-link
 * work for global streaming services (Netflix search, Hotstar, iQIYI, etc.)
 * could ever be reachable, since schedule items never carried non-UK
 * availability data in the first place. Now keeps a curated set of regions.
 * @param {number} tmdbId - TMDB show ID
 * @returns {Promise<Record<string, {flatrate: string[], free: string[]}>|null>}
 */
const TMDB_WATCH_REGIONS = ["GB", "US", "IN", "JP", "FR", "DE", "IT"];
...
export async function fetchTmdbWatchProviders(tmdbId) {
  ...
  try {
    ...
    for (const region of TMDB_WATCH_REGIONS) {
      ...
      if (!entry) continue;
      byRegion[region] = {
        flatrate: (entry.flatrate || []).map((p) => p.provider_name),
        free: (entry.free || []).map((p) => p.provider_name)
      };
    }
...
export async function enrichScheduleWithTmdb(scheduleItems) {
  ...
  return mapWithConcurrency(scheduleItems, MAX_CONCURRENT_LOOKUPS, async (item) => {
    ...
    if (metadata) {
      const providersByRegion = await fetchTmdbWatchProviders(metadata.id);
      const gb = providersByRegion?.GB;
      const streamingOn = gb ? [...gb.free, ...gb.flatrate] : [];
      ...
      return {
        ...
        // GB stays the primary/default surface (existing UI reads these two
        // fields directly) - streamingByRegion carries the rest for anything
        // that wants non-UK availability (e.g. region-aware deep links).
        streamingOn,
        isStreaming: streamingOn.length > 0,
        streamingByRegion: providersByRegion || {}
      };
    }
